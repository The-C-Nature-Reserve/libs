...
static inline void pos(smap_t* m, uint32_t h, char* key)
{
    uint32_t p = h % m->map_alloced;

    while (1) {
        if (m->map[p].pos == SMAP_NULL) {
            break;
        }
        if (m->map[p].hash == h) {
            if (strcmp(m->buf[m->map[p].pos].key, key) == 0) {
                break;
            }
        }
        p = (p + 1) % map_allocated;
...
static inline int check_realloc(smap_t* m)
{
    ...
    // update blocks
    {
        ...
        pair_t* new_map = realloc(m->map, new_map_alloced * sizeof(*new_map));
    ...
    // map used stack to map
    {
        ...
        while (iter != SMAP_NULL) {
            e = &m->buf[iter];
            p = pos(m, e->hash, e->key);
            m->map[p] = { .hash = e->hash, .pos = iter };
...
{
    uint32_t p;
    p = pos(m, HASH(key), key);
    return m->map[p].hash != SMAP_NULL;
}
...
void* smap_insert(smap_t* m, char* key, void* value)
{
    ...
    mp = pos(m, hash, key);
    uint32_t bp = m->map[mp].pos;

    if (bp != SMAP_NULL) {
        m->buf[bp].value = value;
        return value;
    }
...
void* smap_get(smap_t* m, char* key)
{
    ...
    if (m->map[p].hash == SMAP_NULL) {
        return NULL;
    }
    return m->buf[m->map[p].pos].value;
}

void smap_move_pair(smap_t* m, char* key, char** dest_key, void** dest_value)
{
    uint32_t p = pos(m, HASH(key), key);
