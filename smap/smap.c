...
// just to be sure
#define MAP_NULL ((uint32_t)0xFFFFFFFF)
    ...
    uint32_t* buf_pos, uint32_t* chain)
{
    ...
    *chain = MAP_NULL;
    ...
    while (*buf_pos != MAP_NULL && strcmp(m->buf[*buf_pos].key, key) != 0) {
        *chain = *buf_pos;
